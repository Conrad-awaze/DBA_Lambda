...
# $gitRoot                = Split-Path -Path $RootPath -Parent
...
$runtimeBuildScript     = [System.IO.Path]::Combine((Get-Location).Path, 'powershell-runtime', 'build-PwshRuntimeLayer.ps1')
...
$awsToolsBuildScript    = [System.IO.Path]::Combine((Get-Location).Path, 'powershell-modules', 'AWSToolsforPowerShell', 'Demo-AWS.Tools', 'build-AWSToolsLayer.ps1')
